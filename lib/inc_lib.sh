...
#                                   (peut être de la forme login@host:path)
# PARAMETER  3:  exclude_options  : options des fichiers à exclure
#===============================================================================
updateDirectory() {
    ...
    if [[ "${REPERTOIRE_SOURCE}" =~ .+@.+:.+ ]]; then
        if [[ "${REPERTOIRE_DESTINATION}" =~ .+@.+:.+ ]]; then
            ...
            if [[ "${login_host1}" == "${login_host2}" ]]; then
                ...
                verbose_info "-->"${SSH} "${login_host1}" "${RSYNC} -av --ignore-existing ${EXCLUDES} \"${path1}/\" \"${path2}/\""
                ${SSH} "${login_host1}" "${RSYNC} -av --ignore-existing ${EXCLUDES} \"${path1}/\" \"${path2}/\""
                return ${?}
            fi
            verbose_info "-->"${SSH} "${login_host1}" "${RSYNC} -av --ignore-existing ${EXCLUDES} \"${path1}/\" \"${REPERTOIRE_DESTINATION}/\""
            ${SSH} "${login_host1}" "${RSYNC} -av --ignore-existing ${EXCLUDES} \"${path1}/\" \"${REPERTOIRE_DESTINATION}/\""
            return ${?}
        fi
    fi
    verbose_info "-->${RSYNC} -av --ignore-existing ${EXCLUDES} \"${REPERTOIRE_SOURCE}/\" \"${REPERTOIRE_DESTINATION}/\""
    ${RSYNC} -av --ignore-existing ${EXCLUDES} "${REPERTOIRE_SOURCE}/" "${REPERTOIRE_DESTINATION}/"
    return ${?}
}
